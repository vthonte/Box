...
internal const val DEFAULT_SYSTEM_PROMPT =
  """
  You are an on-device assistant that can use skills and tools.
  Return concise final answers. Do not output internal reasoning.

  Tool call format (strict): output ONLY one JSON object:
  {"toolName":"<name>","input":{...}}

  Routing:
  1) If a selected skill matches the task, follow that skill's instructions directly.
  2) Else if an MCP tool matches, call `runMcpTool` with exact `toolName` and valid `input`.
  3) Else return: "No skills or tools found".
...
internal const val DEFAULT_SYSTEM_PROMPT_SKILLS_ONLY =
  """
  You are an on-device assistant that uses selected skills.
  Return concise final answers. Do not output internal reasoning.
  ...
  Flow:
  1) Find the best selected skill from the list below.
  2) If found, follow that skill's instructions directly.
  3) If not found, return: "No relevant skills found".
...
class AgentChatTask @Inject constructor() : CustomTask {
  ...
  override fun initializeModelFn(
    ...
    coroutineScope.launch(Dispatchers.Default) {
      ...
      // Source of truth: DataStore-backed selected skills after loadSkills() completes.
      val selectedSkillsFromStore =
        agentTools.skillManagerViewModel.dataStoreRepository
          .getAllSkills()
          .filter { it.selected }
      val selectedSkills =
        when {
          selectedSkillsFromStore.isNotEmpty() -> selectedSkillsFromStore
          agentTools.selectedSkillsSnapshot.isNotEmpty() -> agentTools.selectedSkillsSnapshot
          else -> agentTools.skillManagerViewModel.getSelectedSkills()
        }
      Log.d(
        TAG,
        "Prompt build: selectedSkills=${selectedSkills.size} names=${selectedSkills.joinToString { it.name }} toolsPromptLength=${toolsPrompt.length} snapshotSize=${agentTools.selectedSkillsSnapshot.size} storeSize=${selectedSkillsFromStore.size}",
      )
...
fun injectSkillsAndMcpTools(
  ...
  val selectedSkillsNamesAndDescriptions =
    skills
      ...
      .joinToString("\n\n") { skill ->
        buildString {
          appendLine("- Skill name: \"${skill.name}\"")
          appendLine("- Description: ${skill.description}")
          appendLine("- Instructions:")
          appendLine(skill.instructions.ifBlank { "(no instructions)" })
        }.trimEnd()
      }
      .ifBlank { "- (no selected skills)" }
